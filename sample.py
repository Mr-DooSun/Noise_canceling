...
filename = ["resource/sample_1.wav","resource/sample_2.wav"]
r = sr.Recognizer()
# 파일이름, 음성인식기를 불러옴

for i in range(0,2):
	try:
	    with sr.AudioFile(filename[i]) as source:  # 오디오 소스를 불러옴
	        audio_data = r.record(source)  # 오디오 소스중 의미있는 부분인 오디오 데이터를 기록함
	        open(filename[i], 'wb').write(audio_data.get_wav_data())  # 오디오 소스를 WAV포맷으로 파일시스템에 전송함
	        freq = filter(filename[i])  # 필터링을 진행함

	    with sr.AudioFile(filename[i]) as source:  # 필터링된 오디오 소스를 불러옴
	        audio_data = r.record(source)  # 오디오 소스중 의미있는 부분인 오디오 데이터를 기록함
	        text = r.recognize_google(audio_data, language='ko-KR')  # 오디오 데이터를 구글음성인식에 전달하여 평문으로 바꿈
	        print(text)  # 텍스트를 콘솔에 출력함

	        if "도와" in text:  # 평문에 해당 글자가 포함되면 파일시스템에 전송함
	            open(f'저장된 호출/sample_{i+1}.wav', 'wb').write(audio_data.get_wav_data())
	            i += 1
	        else:  # 그렇지 않다면 이상없음을 콘솔에 출력함
	            print("이상없음")
